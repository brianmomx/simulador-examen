...
const { Pool } = require('pg'); // Importamos el traductor de PostgreSQL
...
app.post('/registro', async (req, res) => {
    ...
    try {
        ...
        res.status(400).json({ error: "El usuario o correo ya existen." }); 
    }
});
...
app.post('/crear-cuestionario', async (req, res) => {
    ...
    try {
        // En Postgres se usa "RETURNING id" para saber qué número le tocó
...
app.post('/agregar-pregunta', async (req, res) => {
    ...
    try {
        ...
        for(let op of opciones) {
            await pool.query(`INSERT INTO opciones (pregunta_id, texto_opcion, es_correcta) VALUES ($1, $2, $3)`, [pregunta_id, op.texto, op.es_correcta]);
        }
...
// ==========================================
// RUTAS DEL ALUMNO (MOTOR DE EXÁMENES)
// ==========================================
...
app.get('/examen/:cuestionario_id', async (req, res) => {
    ...
    try {
        ...
        // En postgresql pasamos el array directamente con ANY($1)
...
app.post('/guardar-resultado', async (req, res) => {
    const { usuario_id, cuestionario_id, aciertos, total } = req.body;
    try {
        await pool.query(`INSERT INTO resultados (usuario_id, cuestionario_id, aciertos, total_preguntas) VALUES ($1, $2, $3, $4)`, [usuario_id, cuestionario_id, aciertos, total]);
